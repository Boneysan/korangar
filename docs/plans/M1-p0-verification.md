...
| M1-010 | Buff bar shows raw indices, not names | P1 | **Names 2026-07-15/16; icon monograms 2026-07-22:** each line is `[Bl+] Blessing 218s` — two-letter monogram + role tag (`+` buff / `−` debuff / `·` utility). Real GRF SC sprites still deferred (needs licensed artwork). | 🟡 Names + monogram icons; real sprites deferred |
...
| M1-008 | Skills have no visual effect | P1 | **Partial — ongoing with Phase E.** Wizard kit live 2026-07-17; E1 code-drawn Mage/Wizard effects + 0x01F3 `special_effect_recipe` 2026-07-22; Acolyte/Priest holy-hit recipes expanded same day. Still open: full catalog, cast-circle polish, sound retry, E2 units. See [animation-fidelity.md](animation-fidelity.md) §6. | 🟡 Expanded; catalog incomplete by design |
...
| M1-017 | Client crashes on logout | P1 | **Logging out closes the client entirely instead of returning to character select.** Reported live 2026-07-22. Panic: `called \`Option::unwrap()\` on a \`None\` value` at `rust-state/src/state.rs:409:38` = `State::get()`, the **safe** selector path, whose docs say it panics only if a selector is implemented incorrectly or **`ManuallyAssertExt` is used incorrectly**. That points at the player-dependent windows built on `this_player().manually_asserted()`: CharacterOverview (`lib.rs:5039`), Inventory (`5051`), **SkillTree (`5072`)**, Stats (`5081`). Each asserts `this_player` is `Some`; after logout it is `None`, so any such window still rendering unwraps `None`. **Not proven, and one fact cuts against it:** every disconnect/logout path (`2443`, `2556`, `2675`, `2721`) already calls `close_all_windows()` in the same synchronous block as the state clear, so no render should occur in between — meaning either a window survives that close, or the panic is on a different selector entirely. **Not reproduced on a second run** (relaunched with `RUST_BACKTRACE=1`, played to `prt_fild07`, exited with no panic), so the trigger is conditional. The one captured panic has no backtrace — that run predated `RUST_BACKTRACE=1`. Next step: reproduce with the backtrace to name the selector, rather than guessing. Layer: **client UI/state**. Invisible to headless (no UI selectors). | 🔴 Open — mechanism identified, exact selector unconfirmed |
