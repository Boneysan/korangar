...
| M1-010 | Buff bar shows raw indices, not names | P1 | **Names half fixed 2026-07-15/16:** `status_effects.rs` resolves index → English name via `docs/status_effects.json` (699 `SI_*` from Hercules `db/constants.conf`, `tools/export_status_names.py`). Live symptom was Blessing as `10:218s` — now named. **Still open: real SC icons** (needs artwork). Tracked as "real SC icons" in [FEATURE_ROADMAP.md](../FEATURE_ROADMAP.md) §8.3 and [specs/buff-bar-slice.md](../specs/buff-bar-slice.md). | 🟡 Names done; icons open |
...
| M1-008 | Skills have no visual effect | P1 | **Skills deal damage but play no animation.** The packet-side Wizard cause is now proven: Hercules sends Wizard damage through `DisplaySkillEffectAndDamagePacket` (`0x01DE`), but Korangar discarded its `skill_id`. `DamageEffect` now retains optional skill identity. Live result 2026-07-16: Storm Gust ID 89 + `storm_min.str` renders fully. Thunderstorm ID 21 temporarily mapped to that same file also rendered Storm Gust, proving its event/position path, but `cloudh.str` and `lightningstrike\\lightningstrike.str` produced no visible geometry; the false Storm Gust substitute was removed. Initial modern STR choices for Soul Strike/Cold Bolt/Fire Bolt emitted their lights but no geometry; older candidates remain unverified. This exposed a second root cause in the STR renderer, **fixed 2026-07-16**: `Layer::interpolate_frame` ignored the basic/morphing frame-type semantics (morph = per-key deltas + texture cycling via animation type and `delay`), so any base+morph file drew nothing. Rewritten to reference semantics with unit fixtures (`world::effect::frame_at_tests`); `AnimationType::Type4` added; texture index kept fractional. A third root cause landed 2026-07-17: `NotifyGroundSkillPacket` (`ZC_NOTIFY_GROUNDSKILL` 0x0117) was noop, and the original client plays ground-cast area effects (Thunderstorm, Storm Gust) from it at the cast position — now `NetworkEvent::GroundSkillEffect` → `ground_skill_effect()`. Per-hit wiring matches the original (`wizard_hit_effects`: firehit/windhit/lightning bursts) and Fire/Cold Bolt rain classic `FallingBolts` volleys (one projectile per packet `div` hit). **Live-verified 2026-07-17: Thunderstorm, Fire Bolt, Cold Bolt, Storm Gust all visible.** Warp portals additionally render the classic blue vortex (`PortalVortex`, `world/effect/portal.rs`). Remaining under this ID: the **skill-effect coverage pass** (see FEATURE_ROADMAP "Classic skill-effect coverage pass") — same wiring for the rest of the skill catalog, `DisplaySpecialEffectPacket` (`0x01F3`), skill sounds. Follow the No-Upstream-IP rule. Layer: **shared crate + client renderer**. | 🟡 Wizard kit live-verified; catalog-wide coverage pass is scoped future work |
