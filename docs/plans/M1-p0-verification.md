...
| M1-016 | Emotes show no bubble, only chat text | P2 | **Other entities' emotes appear only as a chat line, never the visual bubble.** Found live 2026-07-16 on `prt_fild08` — an emote rendered as `Someone: delighted` in chat but no emoticon drew above the sprite. The machinery exists: `NetworkEvent::DisplayEmotion` (`lib.rs:2857`) calls `EmoteBubbles::show` and lazy-loads the shared emote sprite sheet (`EMOTE_SPRITE_FILE`) via the async loader on first emote; the chat line is explicitly the fallback "until the async load completes" (`world/emote.rs`). So the likely cause is the emote sprite sheet never resolving (`has_animation_data()` stays false → bubbles never render) or a gap in the bubble render path. There's already a debug toggle `emote_debug_enabled()` that logs `data_loaded=` on each emote — start there. The `Someone:` label also means the emoter's name didn't resolve from the entity list. Layer: **client UI/loaders**. Invisible to headless — no scenario asserts on emote bubbles. | 🔴 Open — found 2026-07-16 |
