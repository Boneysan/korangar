...
| M1-015 | Stuck at server select after a failed login | P2 | **Hardened 2026-07-22:** `LoginServerConnectionFailed` now disconnects **login and character** sockets and closes SelectServer / CharacterSelection / CharacterCreation before showing the error, so a stale server-select cannot sit on a dead connection. Original repro was once-only; treat as defensive fix. | ✅ Code complete — live GUI confirm recommended |
