...
- **Inventory management:** Inventory tabs, name search in inventory and storage, sorting, drag arrangement, and server-persisted inventory slot ordering are present. Per-character item protections block NPC sale and dropping through the client actions.
...
- Add a server-authoritative operation to split a selected stack quantity into a second inventory stack. The current drop-quantity operation is not a split operation.
