...
- **The foundation is real.** A custom Rust client connects to a `PACKETVER=20220406` Hercules fork, four friends have played a session over Tailscale from shipped Windows/macOS packs, and the headless protocol suite now registers 151 scenarios. The last complete-suite evidence remains the earlier 149-scenario run; the three inventory scenarios passed in a targeted disposable-server run. Phase 0 and most of the client half of Phase 1 exist.
...
| Test infrastructure            | 151 registered headless scenarios; prior complete-suite evidence is 148 pass / 1 expected skip over 149, plus 3/3 targeted inventory live scenarios on 2026-09-24; skill-expectation generator, observer-parity harness, `playtest-sprite-audit`, 402 client library tests passing (17 ignored). |
