...
| 10.9   | Inventory search / sort / filters       | Inventory has All/Equipped/Gear/Items plus server-item-type Consumables/Etc/Cards/Ammo tabs, search, sort, drag arrangement, and character-persisted item locks/order. | Add quest-item, favorite, and recent-loot semantics when server data supports them; confirm server-authoritative split behavior and live acceptance. | M |
| 10.9   | Storage search                          | Storage has name search plus shared Gear/Items/Consumables/Etc/Cards/Ammo filters. | Add any missing metadata-backed categories and verify storage flows live. | S |
...
| 10.9 Inventory search/filter/sort/lock| Partial — code present, live acceptance open | Inventory has All/Equipped/Gear/Items/Consumables/Etc/Cards/Ammo tabs, name search, sorting, drag arrangement with server-persisted slot ordering, per-character drop/sale protection, and a dedicated server-authoritative split-stack request. Quest-item/favorite/recent-loot semantics need additional data; live split and inventory acceptance remain open. Weight display and thresholds: done. |
| 10.9 Storage                          | Partial — shared filters in source, live acceptance open | Kafra storage open/store/retrieve, name search, and shared Gear/Items/Consumables/Etc/Cards/Ammo filters are present. Live storage transfer/filter acceptance remains open. |
