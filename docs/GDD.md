...
| Status effects      | What each buff/debuff does, duration, what cures it, what causes it, icon.                                           | `sc_config.conf`, `status.c` tables                                                                   | `status_effects.json` has roughly 700 **names only**; effects/cures/sources still need verified data |
...
2. **Player journal.** Promote the DM Bestiary window into a player-visible Adventure Guide with a search field over monsters, items, cards, and the already-exported skills. This is the first shippable slice for build research.

3. **Remaining categories** in order of playtest demand: fuller skill explanations (build questions), maps and quests (navigation, §8.4), verified status mechanics (the Guide currently has icon names only), mechanics, server rules.
...
| 15.1  | Reduced flashing / screen shake      | Persistent `reduce_motion` suppresses the nonessential Magnum Break camera shake; persistent `reduce_flashing` dims procedural burst sprite alpha and associated point lights to 55%, including direct damage-target bursts and `.str` skill lights. Older settings default both controls off. | Extend coverage to authored sprite/particle recipes and other shake sources; verify contrast/readability and visual acceptance. | S-M |
...
| Knowledge Mode             | Open Database: verified reference data searchable | Decided for friends server | Separate player Guide searches monster/item/card/skill/map/job/active-quest references with server-synced account monster and visited-map badges; job-level stat schedules are exported for 147 job IDs. Conditional-script effects, non-active quest coverage, and live account-isolation checks remain open. DM Bestiary unlocks remain session-only. |
...
*2026-09-24 Guide status-name follow-up:* the player Guide now searches the 700-entry status-icon name export by name or icon ID. Detail panels identify the source as a server icon name and explicitly leave effect, duration, sources, interactions, and cures undocumented; no status mechanics are inferred from icon labels.
...
- **§6 (Monster AI) has map-scoped source pilots.** `db/re/mob_ai_profile_db.conf` opts Fabre Aggressor and Poring low-health NPC_RUN on `prt_fild08`, Orc Archer RangedKeeper plus local hazard-weighted A* on `orcsdun02`, and Orc Skeleton three-hit Skirmisher on `orcsdun01`; map+monster lookup prevents global species changes. The C build succeeds, but live farming/retreat/path-load/readability acceptance is outstanding. The wider mid/late family roster and hazard coverage beyond the bounded local path window remain open. Do not confuse client target cycling/monster frame with accepted monster AI.

- **Navigation (§9) and knowledge systems (§9.5, §10.13, §11.5) remain partial.** Route graph/atlas with account-persistent visited markers, searchable graph map identifiers with account-persistent visited badges, validated actionable `<NAVI>` links, Guide/static spawn-map routes, server hunt routes, and item-turn-in→Guide→verified drop-source map routes exist, alongside versioned monster/item/card/skill/job-skill/job-bonus references, 128 inherited job skill trees, 147 job-level stat schedules, active quest search, exported job names, and account-persistent monster milestones. Live account-isolation/reconnect/traversal acceptance remains; NPC/story objective locations without explicit data, conditional spawn coverage, richer build planning/cross-links, and live acceptance remain open.
...
| **Item**                            | **Status**   | **Evidence / Notes**                                                                                                                                                                                                                              |
|-------------------------------------|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 6.2 Behaviour archetypes            | Partial      | Map+monster profile hook in `Hercules/src/map/mob.c`; optional `mob_ai_profile_db.conf` adds Fabre Aggressor/Poring low-health Coward on `prt_fild08`, Orc Archer RangedKeeper on `orcsdun02`, and three-hit Orc Skeleton Skirmisher on `orcsdun01`. Other species/maps remain unchanged; live acceptance remains. Six of eight archetypes are achievable with mode bits + `mob_skill_db` data alone — see §6.10. |
...
| 9.5 In-game encyclopedia          | Partial — player Guide present | Versioned bestiary/item/card/skill data, graph map identifiers, job names, active quests, 128 inherited job-skill trees with validated prerequisites, and 147 job stat-bonus schedules feed a searchable Guide. Job entries link to skills and show cumulative bonuses/milestones; item↔monster and quest↔item/monster cross-links, route actions, source/incomplete-data labels, account-synced badges, and 700 searchable status-icon names are present. Status mechanics, conditional skill effects, richer build planning, mechanics/server rules, authored effects/spawns, and live sync acceptance remain. DM Bestiary/Reveal all remains separate. |
...
| 10.13 Global search                   | Partial — player Guide shipped | Adventure Guide searches monster/item/card/skill/status-name/map/job/active-quest data with route actions for supported records, account discovery badges, clickable item↔monster/quest↔item/monster links, and job→skill cross-links from 128 verified inherited trees; job detail includes 147 exported stat-bonus schedules. Status details currently expose only verified icon names and explicitly mark effects/cures/sources undocumented. Conditional effects, full build planning, and live acceptance remain open. |
...
| 15.1 Colourblind / high-contrast / reduced motion / combat-text controls | Partial | Menu/in-game theme selectors provide built-in High Contrast and Deuteranopia palettes; Deuteranopia changes world self/ally/monster health and cast bars, selected-target bracket, target-frame text, cursor/walk cue, local aim/in-range footprints, incoming cast telegraphs, and party-ping colors. Friend/party state uses blue/neutral/vermillion color tags and explicit status words. Game Settings has persistent Reduce motion and Reduce flashing, quickcast-at-cursor for ground/trap skills, Show combat text, all-vs-important/status-only frequency, and small/normal/large floating-text size controls. Reduced flashing scales procedural burst alpha and associated/skill `.str` point lights to 55%, without shortening durations or removing sounds. Status-only suppresses damage, misses, and healing numbers while preserving textual status notices. Packet-level multi-hit damage appears as one amount/count label. Hold-aim-release, per-skill targeting overrides, authored effect coverage, remaining effect colors, cross-packet hit merging, source-based density, and visual acceptance remain open. |
