...
| 8.5 Shared party progress         | Built; partial stock live check passed | Campaign quest state uses `DM_Party*` helpers. For stock kill objectives, `mob.c` credits nearby members of the killer's party through `quest_update_objective_sub` (`AREA_SIZE`; default 14, active local override 30). Disposable test confirms mixed quest ownership, progress at 30 but not 31 cells, solo credit after leaving the party, and one progress increment per character when both members damage the same monster. Simultaneous separate-monster kills remain unverified. |
