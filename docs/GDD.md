...
| 10.6   | Keybind remap screen                    | Partial: versioned RON overrides drive main UI/navigation shortcuts, WASD movement, all 27 number-row hotbar slots, and debug-camera directional/up movement; Game Settings captures chords, rejects reserved/conflicting keys (allowing same-direction contextual camera/WASD defaults), resets, and imports/exports `client/keybindings.ron`. | Remaining fixed aliases, debug-camera look/acceleration controls, localization, and keyboard-layout/live acceptance. | M |
...
| 15.1  | Alternative ground-target confirm    | Persistent Game Settings can choose quickcast-at-cursor or hold the ground/trap hotbar key while aiming and cast at the current valid cursor target on release; an invalid release cancels, and hold mode takes precedence over quickcast. Existing armed aim-and-click remains the default. | Add optional per-skill override; verify focus/key-release edge cases and complete usability acceptance. | M |
...
- **[partial]** Accessibility pass. *Persistent Reduce motion suppresses the Magnum Break camera shake; persistent Reduce flashing dims procedural combat bursts and associated point lights without shortening timing or removing sounds. Selected monsters now retain a broken-corner, theme-colored bracket around their overhead health indicator; the Deuteranopia palette uses a distinct yellow accent. Ground/trap skills optionally quickcast at the cursor or hold the skill hotbar key while aiming and cast at the current cursor target on release; the default armed aim-and-click flow remains. Show combat text supports off/all/important-only filtering and small/normal/large text; built-in High Contrast and Deuteranopia UI palettes are selectable, and Deuteranopia recolors world self/ally/monster health and cast bars, cursor/walk cue, local aim and incoming cast footprints, and party pings. Friend/party presence and dead-state tags now use blue/neutral/vermillion status colors plus explicit words. Per-skill targeting overrides, authored effect coverage, remaining effect colors, source-based density, and visual acceptance remain open.*
...
*2026-09-24 static warp coverage follow-up:* the navigation exporter now parses warp NPC names containing spaces (including all seven previously omitted multiword-named static warps). Regenerated graph coverage is 538 maps and 3,183 directed edges, with zero unsupported static warp rows; two parser tests cover multiword names, comments, and unknown destination validation. Conditional/scripted transports remain excluded unless separately verified.
...
*2026-09-24 debug-camera accessibility follow-up:* debug-camera forward/back/left/right/up movement is now in the versioned keybinding table and remappable through Game Settings. Existing WASD/Space defaults are preserved; matching WASD movement chords are allowed only for their mutually exclusive debug-camera direction. Keybinding tests cover default collisions, contextual reuse, unrelated conflicts, and remapped dispatch only while camera mode is active. `cargo check -p korangar --features debug` and the focused debug input test pass with `slangc` from `~/.local/opt/slang/bin`; visual and keyboard-layout acceptance remain open. Camera look and Shift-based acceleration/deceleration are still not remappable.
...
*Next work, in order:* (1) close Guide/navigation coverage gaps with verified authored NPC/service destinations and actual quest objective/giver routing data, keeping script references labeled as references; (2) continue remaining accessibility and action-buffer gaps, then retain the visual/usability pass for when requested; (3) run account-discovery isolation/reconnect, party ping/session mixed-client, personal-goal two-character/reconnect/traversal, and S10 campaign replay scenarios against a fully configured server; (4) continue server AI/profile work only after live pilot and path-load acceptance; (5) refresh each GDD row from measured evidence, not source presence alone. The unresolved broad items and their acceptance gates remain tracked in the section tables and `docs/plans/gdd-next-slices.md`.
...
| 5.4 Alternate cast styles                  | Partial          | Classic press-then-click remains; an optional hold-ground/trap-hotbar-key → aim → release-to-cast style and quickcast-at-cursor are available. Per-skill overrides and live/visual acceptance remain open. |
...
| **Item**                          | **Status**   | **Evidence / Notes**                                                                                                                                                                    |
|-----------------------------------|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 9.2 World map                     | Partial — code present, live acceptance open | `World Map & Route Finder` is available from the in-game menu. The generated static-warp graph currently contains 538 maps and 3,183 directed edges, including seven previously omitted multiword-named static warps. The atlas lays out 27 selectable towns/destinations, filters overview connections through graph reachability, highlights a selected route, and shows account-persistent visited badges. Selected-target details show visit-sync state, outgoing exit count, hop count, and next exact portal cell/destination; Towninfo facility actions route to exact client-listed cells. Full travel-service/conditional-route coverage and live acceptance remain open. |
