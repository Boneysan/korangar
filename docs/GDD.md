...
| Party Level Rules          | Friend-friendly widened range / diminishing model | Playtest decision     | Effective import config sets 25% even-share bonus per additional member; `party_share_level: 30` passes focused under/over-boundary live checks with total EXP measurement, but remains a playtest candidate pending group balance acceptance. EXP sharing is same-map rather than cell-distance gated. |
...
*2026-09-25 party EXP live acceptance:* the new `party-experience-sharing` scenario passes against rebuilt Hercules and disposable MariaDB (`runs/20260925-054641.scoped`). Solo Spore EXP measured 117 base / 87 job; with two same-level members sharing 31 cells apart, each received 72/53 (144/106 total, within two points of the configured 25% even-share bonus due to per-member integer rounding). The inclusive 30-level spread shared with both members; 31 levels did not. Re-enabling sharing with an equal-level member on another map yielded no partner EXP. Source and live behavior agree: EXP share is map-scoped, not cell-range-scoped, while quest-credit sharing has its separate 30-cell area check. The 30-level limit remains a playtest candidate, not a final balance decision. Fixture cleanup was clean. The registered suite now has 164 scenarios; a fresh full ordered run remains required after this addition.
...
*2026-09-25 death-recovery threshold follow-up:* `death-recovery-ten-kill-threshold` passed against rebuilt Hercules and disposable MariaDB (`tools/testing/runs/20260925-060751.scoped`). It measures real base/job EXP loss, resurrects the character on the death map with a second client (avoiding the save-point refund path), verifies no refund after kills 1–9, and confirms the exact 50% refund after kill 10. Fixture teardown was clean. Repeated-death accumulation, zero-loss/exempt maps, and party-kill attribution remain open. The headless registry now contains 165 scenarios; a fresh full ordered run is still required.

*2026-09-25 interrupted full-suite follow-up:* the 165-scenario ordered run (`tools/testing/runs/20260925-061047.partial`) was intentionally stopped during phase 9 after `quest-log-multi`; 143 scenarios passed, `skills-professor` failed because two casts had no observable protocol response, and `skills-novice` was the expected skip. The run did not reach the later Guide, inventory split, DM persistence, observer, or death-recovery cases and has no full-suite summary. Hercules and the disposable database were shut down/cleaned by the runner. This partial run is diagnostic only; a complete 165-scenario run remains required. Visual checks remain deferred.
...
| 13.5 Level difference handling    | Partial        | Effective import config gives 25% even-share bonus per extra eligible member; focused rebuilt-server `party-experience-sharing` verifies measured base/job totals, the inclusive 30-level / excluded 31-level boundaries, same-map sharing at 31 cells, and no EXP share across maps. Hercules EXP sharing has no cell-distance check (quest credit does). The 30-level range remains a playtest candidate; no level-sync. |
...
| Test infrastructure            | 165 registered headless scenarios; the latest full ordered run passed **163 non-skipped scenarios with 1 expected skip and 0 flaky passes/failures/unexpected skips**, before the party EXP and death-recovery threshold scenarios were registered. It reported 176 distinct incoming / 68 outgoing packets, zero unknown/fallback/deserialization failures and zero unmet skill expectations; inventory, reset-command behavior, party-quest credit, account discovery after all skill sweeps, navigation, 19-arc DM beats / 116 story beats, death recovery including save-point/refund persistence, and observer cases passed. DM journal/replay and alternate-character SQL audits plus fixture cleanup were clean. Archive: `tools/testing/runs/20260925-042322.log`. Focused reset, party EXP, and death-recovery 10-kill acceptance passed (`20260925-031003.scoped`, `20260925-054641.scoped`, `20260925-060751.scoped`). `cargo test --workspace --quiet` passes (Korangar library: 413 passed, 17 ignored; other test groups also green). A new full ordered run is required against all 165 scenarios; production migration/startup, visual acceptance, and separate playtest/data gaps remain open. |
