...
| Test infrastructure            | 163 registered headless scenarios; the 162-scenario ordered run on 2026-09-25 passed **162 with 1 expected skip and 0 flaky passes/failures**. It reported 176 distinct incoming / 68 outgoing packets, zero unknown/fallback/failed packets and zero unmet skill expectations; inventory, party-quest credit, account discovery after all skill sweeps, navigation, 19-arc DM beats / 116 story beats, death recovery including S4 logout/relogin persistence, and observer cases passed. DM journal/replay and alternate-character SQL audits plus fixture cleanup were clean. Archive: `tools/testing/runs/20260925-014755.log`. The newly registered `reset-command-behavior` scenario passes 1/1 on a rebuilt server (`tools/testing/runs/20260925-031003.scoped`) and confirms free, isolated, relog-persistent stat/skill resets; the ordered suite should be rerun against the 163-scenario registry. `cargo test --workspace --quiet` passes (Korangar library: 413 passed, 17 ignored; other test groups also green). Production migration/startup, visual acceptance, and separate playtest/data gaps remain open. |
