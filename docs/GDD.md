...
| 8.5    | DM Session party story sync          | Party join, quest-log, and map-load hooks reconcile allowlisted campaign state; typed quest/flag events are journaled and replayed with per-character SQL cursors. Replay never grants past rewards. `dm-party-offline-replay` verifies one quest start and flag are replayed after a party member reconnects, and repeated catch-up does not duplicate the active quest notification. | Verify multi-transition cursor order, late join, alternate-character isolation, party recreation, reward isolation, and failed-replay repair; add explicit run identity/repair reporting if party migration testing requires it. [Contract](specs/dm-party-quest-sync.md), S10. | M |
...
| 8.5 DM Session party quest sync    | Partial — offline replay path live-tested | Party-join, quest-log, and map-load hooks retain current-state reconciliation and replay typed quest/flag transitions from a SQL journal using character-keyed cursors, verified postconditions, and no reward replay. `dm-party-offline-replay` verifies one offline party member receives a quest start and flag after reconnect, then repeats catch-up without a duplicate quest notification. Checkpoint mirrors are unprefixed character variables (not account variables). Late join, alternate-character isolation, multi-transition cursor order, reset/recreation, reward isolation, and failure repair remain open. |
...
| Test infrastructure            | 161 registered headless scenarios; prior complete-suite evidence is 148 pass / 1 expected skip over 149, plus targeted inventory live scenarios, account-discovery isolation, party-message carrier, party-quest-credit, reviewed Brasilis quest-NPC interaction, bidirectional navigation portal traversal, and DM offline replay. Latest targeted continuation passes include the full Izlude-to-dungeon route, party carrier sender attribution, and DM quest+flag reconnect replay, with fixture cleanup clean. Skill-expectation generator, observer-parity harness, `playtest-sprite-audit`, and `cargo test --workspace` pass (Korangar library: 412 passed, 17 ignored). |
...
| C7    | Late-joining campaign party members                                | A character joining the active DM Session party automatically inherits its current campaign quests and story flags, without receiving past rewards. State remains per-character; same-account alternates inherit only after joining.                                                                 | 2026-09-23: recorded in the DM party quest-sync contract. S10 source hooks are implemented; 2026-09-24 live reconnect coverage validates one offline quest+flag replay, but late-join and same-account alternate acceptance remain open. |
