...
| 10.13 Global search                   | Partial — player Guide shipped | Adventure Guide searches monster/item/card/skill/status-name/map/job/active-quest data with route actions for supported records, account discovery badges, clickable item↔monster/quest↔item/monster links, and job→skill cross-links from 128 verified inherited trees; job detail includes 147 exported stat-bonus schedules. Status details expose verified status IDs, raw server flags/recalculation fields, and linked associated skills for 540 icon entries; exact effects/cures/sources remain undocumented where no verified data exists. Conditional effects, full build planning, and live acceptance remain open. |
