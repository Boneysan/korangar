...
For this friends server, **both stat and skill points can be reallocated for free at any level by ordinary players**. Experimentation should not require a DM, Zeny, a level/class gate, or an empty inventory. A one-point skill refund remains subject to prerequisite safety checks. This is the target policy, not a claim of current completion: Hercules registers full skill reset as `@skreset`, but group 0 grants `resetskill` and the client sends `@resetskill`; no matching alias was found. The client “Reset stats” button currently sends `@reset`, which resets **both** pools and is not granted to group 0; the stats-only command is `@streset`. The priced Reset Girl script is inactive, while the stock Hypnotist is restricted to eligible first jobs below level 50 with no inventory weight. Align command names/permissions and add obvious player-facing controls for each separate reset (§7.7/S2).
...
| 7.5   | Free player stat/skill reallocation | Hercules registers and group 0 grants `@streset`, `@skreset`, and `@refundskill`; Character Overview now exposes distinct free Stats and Skills reset actions. | Live-verify zero Zeny, unchanged opposite point pool, per-job/level behavior, relog, and one-point refund prerequisite rejection. | partial |
...
| 10.11  | On-HUD quest tracker                 | Track/Untrack, per-character persistence, server hunt counts, and supported map-level routes. Item turn-ins link to Guide item entries and verified drop-source maps. | Add explicit NPC/story objective locations and client-only hunting goals; live-check persistence, routes, and reconnect behavior. | M |
...
| 8.5    | Shared progress for stock quests     | `mob.c` already calls `quest->update_objective_sub` for nearby party members; `quest.c` checks party ID and active quest. Radius is `AREA_SIZE`, currently overridden to 30 cells in `conf/import/battle.conf`. | Live-verify range, duplicate-credit behavior, and quest limits. Add a separate config key only if playtesting shows the draw-distance radius is wrong for quest credit. | verify |
| 8.5    | DM Session party story sync          | Party join, quest-log, and map-load hooks reconcile allowlisted campaign state; typed quest/flag events are now journaled and replayed with per-character SQL cursors. Replay never grants past rewards. | Verify cursor/event migration, reconnect and alternate-character isolation; add explicit run identity/repair reporting if party migration testing requires it. [Contract](specs/dm-party-quest-sync.md), S10. | M |
...
The World Map & Route Finder now shows a partial map-information panel for the selected destination, or the current map when no destination is selected. It reports the mean monster level weighted by exported static spawn-record count, the number of indexed static spawn records and species, verified outgoing portal edges and the next route exit, account visit-sync state, and online party members whose reported map matches. These are reference summaries, not live population counts or a level recommendation; conditional/scripted spawns and NPC/services are not yet included. Missing spawn data is labeled unavailable rather than inferred.
...
| Status effects      | What each buff/debuff does, duration, what cures it, what causes it, icon.                                           | `sc_config.conf`, `skill_db.conf`, `status.c` tables | 700 icon names are searchable; 540 now link to verified server status IDs, raw `sc_config` flags/recalculation fields, and associated skills. Exact outcomes, duration/odds, complete sources, interactions, and cures still need verified data. |
...
| **§** | **Design item**                | **Hook that exists**                                                                                                                                  | **Gap**                                                                                                                                                                                    | **Size** |
|-------|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| 9.2   | World map                      | Generated static-warp graph and in-game atlas/route finder; selected destination details show visit-sync state, known outgoing portal exits, route hop count, and the next verified portal's exact map cell/destination. Current-map walkable breadcrumb routing and Guide/atlas visited badges are present. | Add authored display/service details and broader dynamic/conditional travel coverage; verify live routes. No teleport. | M-L |
| 9.3   | Map information panel          | Atlas panel summarizes exported spawn-record-weighted mean level, static spawn-record/species counts, verified outgoing graph edges/next route exit, visit-sync state, and online party members on the selected/current map. | Add authored NPC/service POIs, world-map danger accent, broader/labelled conditional-spawn coverage, and low-coverage context; then visually/live-check. Data is static and is not a real-time monster count or entry requirement. | M |
...
| 9.6   | Account discovery record       | Hercules stores first-kill milestones in `korangar_account_discovery` keyed by account+mob; account/sequence-bound login snapshots and kill deltas travel in private server messages. Client stages snapshots, preserves prior state until a complete end marker, merges queued deltas monotonically, and labels Guide results discovered/not encountered/sync pending. | Live-check SQL startup/migration, two characters on one account, account isolation, reconnect, and DM separation; add later reviewed encounter milestones. Never gate verified mechanics. | partial |
...
| 10.6   | Keybind remap screen                    | Partial: versioned RON overrides drive main UI/navigation shortcuts, WASD movement, and all 27 number-row hotbar slots; Game Settings captures chords, rejects reserved/conflicting keys, resets, and imports/exports `client/keybindings.ron`. | Remaining fixed aliases, debug-camera controls, localization, and keyboard-layout/live acceptance. | M |
...
| 10.9   | Inventory search / sort / filters       | Inventory has category tabs, search, sort, drag arrangement, and character-persisted item locks/order. | Complete category fidelity, shared inventory/storage filtering, server-authoritative split-to-stack, and live acceptance. | M |
| 10.9   | Storage search                          | Storage has name search and sorting. | Reuse inventory category filters and verify storage flows live. | S |
...
| 15.1  | Combat text size and frequency       | Persistent Game Settings controls hide combat text, filter routine damage while retaining criticals/misses, select a status-only mode with no floating numbers, scale floating text small/normal/large, and show one `amount x hit-count` label for a multi-hit damage packet without summing server values. Textual status notices remain visible in status-only mode. | Add merging across rapid separate packets and visual/readability acceptance. | S-M |
| 15.1  | Alternative ground-target confirm    | Persistent Game Settings can choose quickcast-at-cursor for ground/trap skills; if the cursor has no valid map target, the existing armed aim-and-click flow remains. | Add hold-aim-release and optional per-skill override; complete usability acceptance. | M |
...
- **[partial]** World map and map connection data. *A generated graph indexes loaded static warp scripts; a regional atlas shows 27 selectable towns and destinations, with reachable connections, account-persistent visited markers, and selected route highlighted. The selected-destination panel shows discovery sync state, known outgoing portal count, verified route hop count, and the next exact portal cell/destination. The current map draws a walkable breadcrumb trail to the next portal and recomputes the next leg after a map change. Authored service details, full dynamic/conditional travel coverage, and live acceptance remain open.*
...
| Skill respec               | Free, ordinary-player accessible at every level  | Decided               | Group 0 grants registered `@skreset`/`@refundskill`; Character Overview now has a distinct free skill-reset action. Ordinary-player live verification remains. |
| Stat respec                | Free, ordinary-player accessible at every level  | Decided               | Group 0 grants registered `@streset`; Character Overview now has a distinct free stats-only reset action. Ordinary-player live verification remains. |
...
- Q3–Q4 — **Policy answered; implementation partial.** The player Guide is Open Database by default, with account-wide discovery records independent of reference-field visibility. First-kill account persistence and Guide badges are wired; live two-account/reconnect acceptance remains. DM Bestiary unlocks stay separate and session-only.
...
*2026-09-24 map information/difficulty follow-up:* the atlas panel now reports static-spawn-record-weighted mean level, indexed spawn-record/species counts, verified exits and next route exit, account visit-sync, and online party members reporting that map. Map load presents an optional-to-disable warning when the static mean is at least 15 levels above the character; missing data is silent and travel remains unrestricted. Tests cover the 15-level boundary, missing data, and opt-out. Latest Korangar library run: 384 passed, 17 ignored; workspace check and formatting pass. Danger accents, NPC/service POIs, conditional spawns, and live/visual acceptance remain open.

*2026-09-24 keybinding follow-up:* sustained WASD movement and all 27 number-row hotbar slots now use the versioned remapping table while preserving defaults, F1–F9 aliases, and exact modifier matching. Focused input tests cover remapped hotbar dispatch and retained Shift+1–4 party targeting. Debug-feature compilation was attempted but the environment has no `slangc`, so shader-backed debug build verification remains unavailable. Debug-camera chords, keyboard-layout acceptance, live play, and visual checks are still open.
...
*2026-09-24 timed-buffer follow-up:* when a queued entity target or ground item disappears, the one-slot action buffer is now cleared immediately and a cancellation toast is shown instead of retaining an invalid action until its next processing tick. Unit tests cover entity, pickup-item, and ground-cast invalidation matching; client library suite: 370 passed, 17 ignored. Live input timing acceptance remains open.
...
*2026-09-24 ground-target accessibility follow-up:* Game Settings now persists an optional quickcast-at-cursor mode for ground/trap skills. A valid cursor tile or entity cell casts through the existing walk-into-range path; no valid map target safely falls back to the existing armed aim-and-click behavior. Legacy settings default the option off. Hold-aim-release, per-skill overrides, and visual/usability acceptance remain open.
...
|--------------------------------------------|------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5.2 Input buffer (150-250 ms, one action)  | Partial          | `TimedBufferedAction` is a separate 200 ms, newest-wins slot for attack, pickup, and entity/ground casts during finite local attack/skill/pickup animation. Expiry boundary/wrap and target/item disappearance invalidation are tested; walk chaining/autoattack remain independent. Queue/expiry/cancel toasts provide client feedback. Live lock timing and rapid-input acceptance remain open. |
...
| 7.5 Respec                        | Partial; live check due | Group 0 grants `@streset`, `@skreset`, and `@refundskill`; Character Overview exposes separate free stats/skills reset buttons. Verify ordinary-player behavior, point-pool isolation, relog, and prerequisite rejection. Priced `resetnpc.txt` remains inactive; stock Hypnotist is restricted. |
...
| 8.4 Breadcrumb guidance           | Partial      | World map/next-exit routing draws a sampled walkable minimap breadcrumb and recomputes after map changes. Valid `<NAVI>` targets render labeled route buttons; the atlas exposes exact next-portal cells for verified routes. Identified hunting objectives route only to loaded static spawn maps present in the navigation graph. Non-hunt/NPC objective locations and live traversal remain open. |
...
| 8.5 Shared party progress         | Built; stock live check due | Campaign quest state uses `DM_Party*` helpers. For stock kill objectives, `mob.c` already credits nearby members of the killer's party through `quest_update_objective_sub` (`AREA_SIZE`, effective 30 cells). Quest ownership remains per character. |
| 8.5 DM Session party quest sync    | Partial — event replay source implemented; live acceptance open | Party-join, quest-log, and map-load hooks retain current-state reconciliation and replay typed quest/flag transitions from a SQL journal using character-keyed cursors, verified postconditions, and no reward replay. Checkpoint mirrors are unprefixed character variables (not account variables). Migration/startup, late-join, return, reset/recreation, failure recovery, and character-isolation scenarios still need live acceptance. |
...
| **Item**                          | **Status**   | **Evidence / Notes**                                                                                                                                                                    |
|-----------------------------------|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 9.2 World map                     | Partial — code present, live acceptance open | `World Map & Route Finder` is available from the in-game menu. The generated static-warp graph currently contains 537 maps and 3,176 directed edges. The atlas lays out 27 selectable towns/destinations, filters overview connections through graph reachability, highlights a selected route, and shows account-persistent visited badges. Selected-target details show visit-sync state, outgoing exit count, hop count, and next exact portal cell/destination. Authored services, full travel-service/conditional-route coverage, and live acceptance remain open. |
| 9.3 Map information panel         | Partial — source implemented, visual/live acceptance open | World Map & Route Finder shows spawn-record-weighted mean level, static spawn-record/species counts, graph exit/next-route details, account visit-sync state, and online party members on the selected/current map. No live density count, conditional-spawn coverage, NPC/service POIs, or danger warnings yet. |
...
| 9.5 In-game encyclopedia          | Partial — player Guide present | Versioned bestiary/item/card/skill data, graph map identifiers, job names, account-persistent monster milestones, and tracked-HEAD quest references feed a searchable Guide. The Guide has an All search across monsters/items/cards/skills/statuses/maps/jobs/quests plus per-category filters. Quest search matches titles/IDs, explicit target monsters/maps, and static NPC script references; 842 quest IDs currently have at least one related NPC script reference and supported graph-known NPC cells offer exact-coordinate routes. The references do not prove giver ownership or expose conditional story steps/rewards/prerequisites. Active server quests are merged/labeled; active objectives show progress and link supported monster/map routes. The static quest export deliberately falls back to tracked Hercules HEAD while `db/quest_db.conf` is locally modified and does not read live SQL. Job entries link to skills and show cumulative bonuses/milestones; item↔monster and quest↔item/monster cross-links, route actions, source/incomplete-data labels, account-synced badges, and 700 searchable status-icon names are present. 540 status icons expose linked server status IDs, raw flags/recalculation metadata, and associated-skill links. Exact status outcomes/durations/cures, conditional skill effects, richer build planning, mechanics/server rules, authored effects/spawns, and live sync acceptance remain. DM Bestiary/Reveal all remains separate. |
...
| **Item**                              | **Status**   | **Evidence / Notes**                                                                                                                                                          |
|---------------------------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 10.2 HUD presets / Edit Mode          | Partial      | Versioned `WindowCache` migrates legacy geometry and stores movement lock plus named per-character layouts. Game Settings exposes lock/unlock, Classic/Modern presets, `My Layout`, and reset. Combat-only fading, configurable snap behavior, and visual acceptance remain open. |
...
| 10.6 Hotbars                          | Partial      | Three rows of nine (1-9, Ctrl+1-9, Alt+1-9; F1–F9 aliases still work); items/potions on the hotbar (2026-09-05). All 27 number-row slots now use the versioned Game Settings remapping table. Debug-camera bindings and live/alternate-layout acceptance remain open. |
| 10.7 Equipment sets                   | Not started  |                                                                                                                                                                               |
| 10.8 Stats interface modes            | Not started  |                                                                                                                                                                               |
| 10.9 Inventory search/filter/sort/lock| Partial — code present, live acceptance open | Inventory has All/Equipped/Gear/Items tabs, name search, sorting, drag arrangement with server-persisted slot ordering, and per-character drop/sale protection. Fine-grained categories and server-authoritative split-to-stack remain open. Weight display and thresholds: done. |
| 10.9 Storage                          | Partial      | Kafra storage open/store/retrieve and name search are present. Category filters and live acceptance remain open. |
...
| 10.11 Quest tracker                   | Partial      | Quest log has Track/Untrack, a HUD summary, and per-character persistence reconciled against the server list. Hunting notifications/progress populate mob counts and link known mobs to static spawn-map routes. Item turn-ins open the matching Guide item/card record and link to graph-known verified drop-source maps. Explicit NPC/story locations, client-only hunting goals, and live reconnect/traversal acceptance remain open. |
| 10.12 Minimap layers / waypoints / pings | Partial | Minimap offers six expiring party ping kinds over v2 chat, retains v1 location decoding, checks sender/map/cells, and draws one same-map marker on the minimap and in-world; Hercules source bounds/rate-limits ping and shared-route/ready-check messages. Graph routing and visited badges are present. Ready checks are nonce-bound with one response per online roster member and 30-second expiry. Personal waypoints, mixed-client/live acceptance, and marker layers remain open. |
| 10.13 Global search                   | Partial — player Guide shipped | Adventure Guide has a combined All search for monster/item/card/skill/status/map/job/quest data and per-category filters. Quest search includes target monsters/maps, 842 related NPC script references, and active server entries; graph-known NPC cells offer exact-coordinate route actions. Account discovery badges, clickable item↔monster/quest↔item/monster links, and job→skill cross-links from 128 verified inherited trees are present; job detail includes 147 exported stat-bonus schedules. Status details expose verified status IDs, raw server flags/recalculation fields, and linked associated skills for 540 icon entries; exact effects/cures/sources remain undocumented where no verified data exists. Script references do not prove quest-giver relationships or disclose conditional story steps, rewards, or prerequisites; those, conditional effects, full build planning, and live acceptance remain open. |
...
| 14.3 Difficulty communication | Partial — client warning implemented, live/visual acceptance open | Atlas shows static-spawn-record-weighted mean level; map-load warning toast triggers at 15 levels above character and can be disabled in Game Settings. It never blocks entry. World-map danger accent and live timing/readability acceptance remain. |
...
| 15.1 Remappable controls                   | Partial      | Versioned settings and a Game Settings capture list cover primary UI/navigation actions, 27 hotbar slots, and movement with conflict/reserved-key checks, reset, and RON import/export. Debug-camera controls and full keyboard-layout acceptance remain. |
| 15.1 Colourblind / high-contrast / reduced motion / combat-text controls | Partial | Menu/in-game theme selectors provide built-in High Contrast and Deuteranopia palettes; Deuteranopia changes world self/ally/monster health and cast bars, selected-target bracket, target-frame text, cursor/walk cue, local aim/in-range footprints, incoming cast telegraphs, and party-ping colors. Friend/party state uses blue/neutral/vermillion color tags plus explicit status words. Game Settings has persistent Reduce motion, Reduce flashing, quest-marker visibility, quickcast-at-cursor for ground/trap skills, Show combat text, all-vs-important/status-only frequency, and small/normal/large floating-text size controls. Reduced flashing dims procedural and ACT sprite-effect alpha plus associated/skill `.str` point lights to 55%, without shortening durations or removing sounds. Status-only suppresses damage, misses, and healing numbers while preserving textual status notices. Packet-level multi-hit damage appears as one amount/count label. Hold-aim-release, per-skill targeting overrides, remaining particle/effect coverage, remaining effect colors, cross-packet hit merging, source-based density, and visual acceptance remain open. |
...
| 16.3 Combat text options                   | Partial      | Persistent on/off, all-vs-important/status-only filtering, and Small/Normal/Large size controls affect floating damage, miss, and healing numbers. Multi-hit packets use one amount/count label; merging across separate packets remains open. |
...
| C7    | Late-joining campaign party members                                | A character joining the active DM Session party automatically inherits its current campaign quests and story flags, without receiving past rewards. State remains per-character; same-account alternates inherit only after joining.                                                                 | 2026-09-23: recorded in the DM party quest-sync contract. S10 implements and verifies the snapshot/replay behavior.                                                                                 |
...
- **DM group quest sync (2026-09-23; implementation follow-up 2026-09-24):** A character joining the active campaign party automatically inherits its current campaign quests and story flags, without past rewards. A disconnected member catches up on return; another character on the same account inherits nothing until it joins. S10 now journals typed events and replays them using character-scoped cursors, alongside snapshot catch-up; run identity and live character isolation remain unaccepted.
