...
| 8.5    | Shared progress for stock quests     | `mob.c` calls `quest->update_objective_sub` for nearby party members; `quest.c` checks party ID and active quest. Radius is `AREA_SIZE`, with a current source default of 14 cells in `conf/map/battle/client.conf` (server overrides may change it). | Disposable live scenario verifies nearby shared credit and per-character quest ownership. Exact range boundary, solo credit, duplicate-attribution behavior, and simultaneous kills remain to be tested. | partial |
...
*2026-09-24 account-discovery live follow-up:* `account-discovery-isolation` passed against a freshly rebuilt Hercules and disposable MariaDB. It verified a first-kill delta, replay of that mob in a second character's completed snapshot on the same account, and an empty snapshot for a separate account; fixture cleanup was clean. This closes the core account-isolation/reconnect acceptance for monster milestones. Production migration/startup and additional discovery types remain open.

*2026-09-24 party-session carrier follow-up:* the two-client carrier scenario now also verifies that Hercules relays `[KORANGAR-SESSION:v1]` ready-check traffic, applies the same per-character cooldown across ping and session message kinds, accepts a 128-byte session body, and rejects 129 bytes. The disposable server run passed with clean fixture teardown. This proves carrier bounds/relay, not the client's shared-route and ready-check state handling, stock-client presentation, or reconnect/map-change behavior.

*2026-09-24 stock quest-credit follow-up:* new disposable live scenario `party-quest-credit` passed against two Korangar clients and the server's Spore hunt objective. A member with the active quest received kill progress, a nearby party member without the quest received none, and after independently adding the quest that member received progress on the next party kill. Fixture cleanup was clean. The GDD's former 30-cell override claim was stale: current Hercules source defaults `AREA_SIZE` to 14 cells in `conf/map/battle/client.conf`; server overrides may change it. Exact boundary, solo, duplicate, and simultaneous-kill checks remain open.
...
| 8.5 Shared party progress         | Built; partial stock live check passed | Campaign quest state uses `DM_Party*` helpers. For stock kill objectives, `mob.c` credits nearby members of the killer's party through `quest_update_objective_sub` (`AREA_SIZE`, source default 14 cells, overridable). Disposable test confirms credit for active nearby quest owners and no credit for a party member without that quest; range boundary, solo, duplicate, and simultaneous-kill behavior remain unverified. |
...
| 9.6 Account discovery             | Partial; core live acceptance passed | Hercules has account+mob and account+map SQL ledgers with first-kill/first-visit deltas and complete bounded login snapshots; Korangar account-binds private server messages and shows Guide badges without hiding facts. Disposable live acceptance passed for first-kill delta delivery, same-account alternate-character snapshot replay, and isolation from a separate account's empty snapshot. Broader encounter milestones and production migration/startup acceptance remain. DM unlocks stay session-only; `@monster` is server permission-checked. |
...
| Test infrastructure            | 157 registered headless scenarios; prior complete-suite evidence is 148 pass / 1 expected skip over 149, plus 2/2 targeted inventory live scenarios, account-discovery isolation, party-message carrier, and party-quest-credit checks on 2026-09-24; skill-expectation generator, observer-parity harness, `playtest-sprite-audit`, full workspace tests pass (latest Korangar library 405 passed, 17 ignored). |
