...
The World Map & Route Finder now shows a partial map-information panel for the selected destination, or the current map when no destination is selected. It reports the mean monster level weighted by exported static spawn-record count, indexed static spawn-record/species counts, verified outgoing portal edges and the next route exit, account visit-sync state, and online party members whose reported map matches. When the selected map is also the current map, it lists facility names from the client's Towninfo table; remote destinations explicitly say those facilities are not loaded. These are reference summaries, not live population counts or a level recommendation; conditional/scripted spawns and broader NPC/service coverage are not included. Missing spawn data is labeled unavailable rather than inferred.
...
| Maps                | Name, region, suggested level, connections (warp graph), monster population, NPCs and services, Kafra, map flags.    | `db/re/map_zone_db.conf`, `npc/**/warps/*.txt`, `npc/**/mobs/*.txt`, `Towninfo`                        | Guide details graph maps with static-spawn mean/counts and verified outgoing exits/routes; atlas adds account visits, danger marker, party presence, and current-map Towninfo facility names. Remote/authored services, map flags, conditional spawns remain incomplete. |
...
*2026-09-24 map information/difficulty follow-up:* the atlas panel and Guide map details now report static-spawn-record-weighted mean level, indexed spawn-record/species counts, verified exits/routes, account visit-sync, and (in the atlas) online party members reporting that map. Guide map details list up to eight outgoing portal cells and route actions. For the current map, atlas details also list facility names from the client Towninfo table; remote maps explicitly report those facilities as not loaded. Map load presents an optional-to-disable warning when the static mean is at least 15 levels above the character; atlas nodes at that threshold get a contrasting outline and `!`. Missing data is silent and travel remains unrestricted. Tests cover map summaries, route details, POI availability labels, the 15-level boundary, missing data, and opt-out; the shared threshold is unit-tested. Latest Korangar library run: 385 passed, 17 ignored; workspace check and formatting pass. Remote/authored service POIs, conditional spawns, low-coverage context, and live/visual acceptance remain open.
...
| 9.3 Map information panel         | Partial — source implemented, visual/live acceptance open | World Map & Route Finder shows spawn-record-weighted mean level, static spawn-record/species counts, graph exit/next-route details, account visit-sync state, and online party members on the selected/current map. It lists Towninfo facility names only when the selected map is current; remote facility data is unavailable. No live density count or conditional-spawn coverage. Danger markers and a configurable non-blocking entry warning exist; NPC/service coverage remains partial. |
