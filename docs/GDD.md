...
*2026-09-25 live-server acceptance:* a full ordered run against freshly provisioned Hercules and MariaDB passed **162/162 non-skipped scenarios**, with one expected `skills-novice` skip, zero failures/flaky retries/unexpected skips, 176 distinct incoming and 68 outgoing packets, zero unknown/fallback or failed packets, zero unmet skill expectations, clean fixture cleanup, and clean DM journal/replay cursor audits. Archive: [`runs/20260925-003101.log`](../tools/testing/runs/20260925-003101.log). This run includes `account-discovery-isolation` after the long skill sweep and `party-quest-credit`, plus `death-recovery-save-point`. The discovery fixture now logs in directly instead of using the paired-venue warp (which flushes pending login snapshots) and waits for both account-ledger end markers before selecting unused mob/map fixtures. This closes the full ordered headless-server registry gate; it does not replace production migration/startup, live graphical playtests, or the separately tracked gameplay/data gaps. Visual acceptance remains deliberately deferred.

*2026-09-25 death-recovery persistence follow-up:* the S4 scenario now disconnects after observing real base/job EXP loss, reconnects the same character, then verifies the exact measured half-refund on save-point arrival with no duplicate refund. This directly validates persistence of the pending per-character ledger across logout/relogin. Focused rebuilt-server run passed 1/1 with clean fixture teardown (`tools/testing/runs/20260925-014233.scoped`). The earlier full ordered run exercised save-point recovery without this added relog interval; repeated-death accumulation, zero-loss/exempt maps, party-kill attribution, and the 10-kill threshold remain open.
...
| Test infrastructure            | 162 registered headless scenarios; latest ordered full run (2026-09-25): **162 passed, 1 expected skip, 0 failed**. Packet coverage: 176 distinct incoming / 68 outgoing; zero unknown/fallback/failed packets and zero unmet skill expectations. Inventory, party-quest credit, account discovery after all skill sweeps, navigation, 19-arc DM beats / 116 story beats, death recovery, and observer cases passed; DM journal/replay and alternate-character SQL audits plus fixture cleanup were clean. Archive: `tools/testing/runs/20260925-003101.log`. The later S4 logout/relogin persistence assertion passes in a focused rebuilt-server run (1/1; clean fixture teardown). `cargo test --workspace --quiet` passes (Korangar library: 413 passed, 17 ignored; other test groups also green). This closes the full ordered headless registry gate only; production migration/startup, visual acceptance, and separate playtest/data gaps remain open. |
