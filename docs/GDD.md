...
*2026-09-24 debug-camera accessibility follow-up:* debug-camera forward/back/left/right/up movement is now in the versioned keybinding table and remappable through Game Settings. Existing WASD/Space defaults are preserved; matching WASD movement chords are allowed only for their mutually exclusive debug-camera direction. Keybinding tests cover default collisions, contextual reuse, and unrelated conflicts. The focused seven-test keybinding group and dedicated collision test pass. A debug-feature compile was attempted but its build script requires `slangc` (>= 2025.18.2), unavailable in this environment; visual and keyboard-layout acceptance remain open. Camera look and Shift-based acceleration/deceleration are still not remappable.
...
| 8.4 Breadcrumb guidance           | Partial      | World map/next-exit routing draws a sampled walkable minimap breadcrumb and recomputes after map changes. Valid `<NAVI>` targets render labeled route buttons; the atlas exposes exact next-portal cells for verified routes and Towninfo facility actions route to their exact client-listed cells. Identified hunting objectives route only to loaded static spawn maps present in the navigation graph. Non-hunt/story objective locations and live traversal remain open. |
