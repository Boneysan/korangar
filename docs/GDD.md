...
*2026-09-24 live server follow-up:* after the account-discovery collector was corrected to observe map-change and discovery events together, `account-discovery-isolation` passed in a fresh rebuilt-server run. The new `death-recovery-save-point` scenario exposed that Hercules invokes `OnPCLoadMapEvent` only on maps carrying the global `loadevent` mapflag; normal save-point arrivals therefore skipped recovery. Map transitions now dispatch the scoped `KorangarDeathRecovery::OnKorangarMapChange` event, leaving unrelated map-load scripts opt-in. Rebuilt Hercules passed both focused scenarios (2/2; 0 unknown/fallback/failed packets; fixture cleanup clean). This covers the scripted EXP-loss and save-point refund path, not every penalty map, job-EXP edge, repeated death, or production rollout. The last full ordered suite was 160 pass / 1 failure / 1 expected skip; its failure was the discovery event-order race fixed here. Repeat the full ordered suite before calling the registry accepted. Visual acceptance remains deliberately deferred.
...
| Test infrastructure            | 162 registered headless scenarios; latest ordered full run: 160 pass / 1 failure / 1 expected skip. The failure was a discovery event-order race; after fixing it, focused rebuilt-server runs passed account-discovery isolation and death-recovery save-point (latest: 2/2, zero unknown/fallback/failed packets, clean fixture audit). Earlier focused rebuilt-server run passed the five dialogue cases, reviewed Brasilis routing, and discovery (7/7). All other full-run scenarios passed, including inventory, party, navigation, and DM reconnect/alternate-character paths; DM SQL cursors reached the journal tail. Full ordered rerun remains due. Latest recorded `cargo test --workspace` passed (Korangar library: 413 passed, 17 ignored); rerun after current source changes. |
