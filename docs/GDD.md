...
| Test infrastructure            | 162 registered headless scenarios; the 2026-09-24 ordered full run had 156 pass / 5 fixture failures / 1 expected skip. After correcting the NPC search overlap and current-map discovery fixture, all five failed cases passed in a focused rebuilt-server run alongside reviewed Brasilis routing (7/7, zero unknown/fallback/failed packets). Thus all 161 non-skipped scenarios are green across combined full and targeted evidence; one full ordered rerun remains due. The run also passed targeted inventory split/live paths, party-message carrier, party-quest-credit, navigation traversal, and DM reconnect/alternate-character paths; DM SQL cursors reached the journal tail and fixture cleanup audits were clean. `cargo test --workspace` passes (Korangar library: 413 passed, 17 ignored). |
