...
| **§** | **Design item**                | **Hook that exists**                                                                                                                                  | **Gap**                                                                                                                                                                                    | **Size** |
|-------|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| 9.2   | World map                      | Generated static-warp graph and in-game atlas/route finder; selected destination details show visit-sync state, known outgoing portal exits, route hop count, and the next verified portal's exact map cell/destination. Current-map walkable breadcrumb routing and Guide/atlas visited badges are present. The headless live test traversed Prontera ↔ `prt_fild08` in both directions through walk-triggered warps on PACKETVER 20220406; live NPC dialogue also verifies Izlude's 150-zenny Sailor transfer to `izlu2dun`. | Still model NPC/service hops in routable details; the `izlu2dun` → `iz_dun00` walk warp currently returns no movement acknowledgement in the headless fixture and needs investigation; client route following and visual acceptance remain. No teleport. | M-L |
...
| 9.6 Account discovery             | Partial; monster + map live acceptance passed | Hercules has account+mob and account+map SQL ledgers with first-kill/first-visit deltas and complete bounded login snapshots; a dedicated map-change event records visits without globally enabling map `loadevent` scripts. Korangar account-binds private server messages and shows Guide badges without hiding facts. Disposable live acceptance passed for monster/map deltas, same-account alternate-character snapshots, and isolation from a second account. Production migration/startup and broader encounter milestones remain. DM unlocks stay session-only; `@monster` is server permission-checked. |
