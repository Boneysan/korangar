...
| **§** | **Design item**                | **Hook that exists**                                                                                                                                  | **Gap**                                                                                                                                                                                    | **Size** |
|-------|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| 9.2   | World map                      | Generated static-warp graph and in-game atlas/route finder; selected destination details show visit-sync state, known outgoing portal exits, route hop count, and the next verified portal's exact map cell/destination. Current-map walkable breadcrumb routing and Guide/atlas visited badges are present. A bidirectional Prontera ↔ `prt_fild08` headless server-traversal scenario is registered; live acceptance is still pending. | Add authored display/service details and broader dynamic/conditional travel coverage; run the live traversal and continue visual acceptance. No teleport. | M-L |
...
| Test infrastructure            | 158 registered headless scenarios; prior complete-suite evidence is 148 pass / 1 expected skip over 149, plus 2/2 targeted inventory live scenarios, account-discovery isolation, party-message carrier, and party-quest-credit checks on 2026-09-24; navigation portal traversal is now registered but not yet live-run (host `mysql` CLI unavailable). Skill-expectation generator, observer-parity harness, `playtest-sprite-audit`, full workspace tests pass (latest Korangar library 405 passed, 17 ignored). |
