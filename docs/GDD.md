...
| Test infrastructure            | 162 registered headless scenarios; prior complete-suite evidence is 148 pass / 1 expected skip over 149, plus targeted inventory live scenarios, account-discovery isolation, party-message carrier, party-quest-credit, reviewed Brasilis quest-NPC interaction, bidirectional navigation portal traversal, and DM party reconnect/alternate-character paths. Both DM party scenarios pass together in runner order with SQL assertions for typed events, enrollment, and actor/returning cursors reaching the journal tail; fixture cleanup is clean. `cargo test --workspace` passes (Korangar library: 412 passed, 17 ignored); full headless-suite rerun remains due. |
