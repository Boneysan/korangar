...
- **Party context**: Campaign is party-locked on the server (`$dm_active_party`, `DM_RequireDM`). See dedicated targeted spec `docs/specs/party-packets.md` (layouts for 0x0AE4/0x0AE5 + 0x09DE whisper, steps, events). Party roster / HP / position packets are still mostly noop today.
