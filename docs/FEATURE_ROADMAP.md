...
- [ ] **Combat feedback & readability:**
  ...
  - **Emote bubbles** *(implemented 2026-07-13)*: `DisplayEmotion` events play
    ...
    ID→action alignment across the full emote range (dice, flags). Per-entity
    height anchoring and an 81-entry `Alt+L` picker landed 2026-07-16.
...
| ~~**MVP**~~ **Partial 2026-07-09** | Skill/damage feedback → floating combat text, cooldowns (§8 Combat) | **Done:** `DisplaySkillEffectAndDamagePacket` → floating numbers; `DisplayPlayerHealEffect` → heal numbers. **Promoted 2026-07-17:** `NotifyGroundSkillPacket` → `GroundSkillEffect` (ground-cast area effects — this was why Thunderstorm showed nothing; see the "Classic skill-effect coverage pass" item below). **Still noop:** `DisplaySkillCooldownPacket` (hotbar polish), `DisplaySpecialEffectPacket`, `UseSkillSuccessPacket` (cast bars). Failure path of `ToUseSkillSuccessPacket` already promoted (server-feedback row). |
| **High** *(partial 2026-07-17)* | **Classic skill-effect coverage pass** — audit the rest of the skill catalog for the three wiring gaps the Wizard kit hit (§8 Combat, M1-008) | **Batches 1–2 implemented:** `skill_hit_effects()` covers additional Mage/Wizard hits plus Knight, Priest, Hunter, Assassin, and Holy Light STRs; `ground_skill_effect()` adds Firewall, Sanctuary, Magnus, Fire Pillar, Meteor Storm, Lord of Vermilion, Quagmire, Hammer Fall, Skid Trap, and Venom Dust. Meteor and Frost Nova are live-verified with correct ground/caster triggers. The successful-use caster path now also covers Magnum Break, Raid, Meteor Assault, and Ignition Break; `SkillBurst` supplies their classic procedural cylinder/streak/slash recipes where no STR exists. Damage-triggered, deduplicated caster layers add the missing Pierce, Brandish Spear, Spear Stab, Spear Boomerang, Bowling Bash, and Sonic Blow presentation. The six-skill Knight bar now has its equipped weapon ACT, body/head offsets, target/projectile layers, point lights, original sounds, and Magnum Break quake; see [KNIGHT_SKILL_EFFECT_RECIPES.md](KNIGHT_SKILL_EFFECT_RECIPES.md). Entity-targeted skills also path into their learned range and cast on arrival instead of silently failing. All referenced Knight assets passed a configured-GRF audit. A repeatable four-character GUI roster plus `provision-effect-roster` scenario verifies every required skill and stocks its weapons. **Still open:** GUI acceptance of every non-Knight batch-2 recipe; other code-drawn effects (Soul Strike orbs, Frost Diver travel, Napalm, Jupitel, Earth Spike/Heaven's Drive geometry); persistent skill-unit visuals beyond Firewall/Pneuma (Safety Wall, Sanctuary, Magnus, Sage fields, songs, Ice Wall…); `DisplaySpecialEffectPacket` (0x01F3); cast circles; and catalog-wide classic sounds beyond this Knight pass. Proven method: focused wire probe → `str_frame_structure` dump → roBrowserLegacy semantic cross-check → live check. |
...
| ~~High~~ **Done 2026-07-08** (+ table 2026-07-10; skill-fail closed 2026-07-22) | Server feedback → visible rejection messages | `MessageTablePacket` / `MessageTableColorPacket` → `NetworkEvent::MessageTable`; client resolves via `MsgStringTable` (`archive/data/msgstringtable.txt`, 0-based). Fixes boot id 3474 (`MSG_CHECK_ATTENDANCE_NOT_EVENT`). Skill-fail (`ZC_ACK_TOUSESKILL` 0x0110 → `skill_failed_text` chat) unit-tested + headless `skill-fail-rejection` green. |
