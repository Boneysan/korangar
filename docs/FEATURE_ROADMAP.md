...
| **High** *(partial 2026-07-17)* | **Classic skill-effect coverage pass** — audit the rest of the skill catalog for the three wiring gaps the Wizard kit hit (§8 Combat, M1-008) | **Asset-backed batch 1 implemented:** `skill_hit_effects()` now covers additional Mage/Wizard hits plus Knight, Priest, Hunter, Assassin, and Holy Light STRs; `ground_skill_effect()` adds Firewall, Sanctuary, Magnus, Fire Pillar, Meteor Storm, Lord of Vermilion, Quagmire, Hammer Fall, Skid Trap, and Venom Dust. Meteor's full `meteor1-4.str` plays from the ground-cast notification and its per-target damage adds only `firehit1-3.str`. Frost Nova plays once on the caster from the successful non-damage skill-use packet, including empty-area casts. Both are live-verified. All mapped effects use `EffectWithLight` with elemental colors; all referenced STRs parse from the configured GRFs. **Still open:** code-drawn effects (Soul Strike orbs, Frost Diver travel, Napalm, Jupitel, Earth Spike/Heaven's Drive geometry), persistent skill-unit visuals beyond Firewall/Pneuma (Safety Wall, Sanctuary, Magnus, Ice Wall…), `DisplaySpecialEffectPacket` (0x01F3), cast circles, classic sounds, and live GUI verification of the remaining batch. Proven method: focused wire probe → `str_frame_structure` dump → roBrowserLegacy semantic cross-check → live check. |
