...
| **High** *(added 2026-07-17)* | **Classic skill-effect coverage pass** — audit the rest of the skill catalog for the three wiring gaps the Wizard kit hit (§8 Combat, M1-008) | The Wizard fixes exposed three *classes* of missing wiring, each of which silently produces "skill shows nothing/wrong": **(1) ground-cast STRs** — only skills 21/89 are in `ground_skill_effect()`; every other ground cast (Firewall cast flash, Meteor, Lord of Vermilion, Sanctuary, Magnus, Quagmire, Fire Pillar, traps…) arrives via the now-promoted `GroundSkillEffect` and needs a mapping (roBrowserLegacy `SkillEffect.js` `effectId` fields are the reference); **(2) per-hit STRs** — `wizard_hit_effects()` covers 13/19/20/21 only; melee/holy/etc. hits (`hitEffectId`: firehit/windhit/holyhit/`hit.str` families, all present in data.grf) need table rows; **(3) code-drawn effects** — bolts got `FallingBolts`; Soul Strike orbs, Frost Diver's ice trail, Napalm Beat, etc. were also never STRs (roBrowserLegacy `ef_*` entries give the recipes). Also fold in: skill-unit visuals beyond Firewall/Pneuma (`UnitId` match in lib.rs — Safety Wall, Sanctuary, Ice Wall…), `DisplaySpecialEffectPacket` (0x01F3, the 1124-entry effect-id table), cast circles (`spell.str` on `UseSkillSuccessPacket`), and classic per-effect **sounds** (the tables name the exact wavs). Method that worked: probe the wire with a focused headless scenario (`probe-thunderstorm` pattern), dump candidate STRs with `str_frame_structure`, cross-check roBrowserLegacy tables, verify live. |
