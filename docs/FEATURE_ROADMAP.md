...
| ~~**MVP**~~ **Partial 2026-07-09** | Skill/damage feedback → floating combat text, cooldowns (§8 Combat) | **Done:** `DisplaySkillEffectAndDamagePacket` → floating numbers; `DisplayPlayerHealEffect` → heal numbers. **Promoted 2026-07-17:** `NotifyGroundSkillPacket` → `GroundSkillEffect`. **Promoted 2026-07-22:** `DisplaySpecialEffectPacket` (0x01F3) → `SpecialEffect` + `special_effect_recipe` (E1 + common IDs). **Still noop:** `DisplaySkillCooldownPacket` (hotbar polish), `UseSkillSuccessPacket` (cast bars). Failure path of `ToUseSkillSuccessPacket` already promoted (server-feedback row). |
