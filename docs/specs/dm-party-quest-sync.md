...
- The event tables are declared in `2026-09-24--campaign-event-replay.sql` and are also created idempotently by the DM event NPC at startup. Run identity remains the fixed campaign namespace plus party ID; party re-creation migrates the journal when no destination checkpoint exists, and explicit reset clears the journal/cursors. A separate run UUID and repair/preview UX are not implemented. `dm_campaign_checkpoint_*` variables in `dm_checkpoint.txt` are unprefixed Hercules character variables (the SQL member row is also keyed by character ID); the older claim that these mirrors were `#` account-scoped was incorrect. Live alternate-character isolation still needs acceptance.

Automatic status inheritance and source-side typed replay are present but remain **unverified in a live two-character session**. The older 20 September acceptance checklist records the late-join scenario as open until it is played.
