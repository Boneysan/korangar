    ...
    B: Element<ClientState>,
{
    ...
    fn create_layout_info(
        ...
        with_single_resolver(resolvers, |resolver| {
            ...
            self.price_string.update(item.price.0 * item.metadata.1 as u32);
                    ...
                    for index in self.elements.len()..cart.len() {
                            ...
                            B: Path<ClientState, Vec<SellItem<(ResourceMetadata, u16)>>>,
                        {
                            fn handle_click(&self, state: &State<ClientState>, _: &mut EventQueue<ClientState>) {
                                ...
                                state.update_value_with(self.cart_path, move |cart| {
                                    if let Some(index) = cart.iter_mut().position(|purchase| purchase.inventory_index == inventory_index) {
                        ...
                        let buttons = (split! {
                            ...
                            children: (
                                ...
                                button! {
                                    text: "-All",
                                    event: RemoveAction::new(item_path, self.cart_path, u16::MAX),
                                },
                            ),
                        },);
    ...
    fn to_window<'a>(self) -> impl Window<ClientState> + 'a {
        ...
        window! {
            ...
            elements: (
                    ...
                    children: (
                        button! {
                            ...
                            event: move |state: &State<ClientState>, queue: &mut EventQueue<ClientState>| {
                                ...
                                queue.queue(InputEvent::SellItems { items });
                            }
                        },
                        button! {
                            text: "Cancel",
                ...
                scroll_view! {
                    children: ItemList::new(self.cart_path),
                },
            ),
        }
    }
